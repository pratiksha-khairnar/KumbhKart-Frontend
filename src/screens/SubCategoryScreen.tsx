...
//           <View style={[styles.aboutDropdownMenu, { top: aboutBtnLayout.y + aboutBtnLayout.height + 4, left: aboutBtnLayout.x }]}>
//             {ABOUT_DROPDOWN.map((item, index) => (
//               <TouchableOpacity
//                 key={item.label}
//                 style={[styles.aboutDropdownItem, index < ABOUT_DROPDOWN.length - 1 && styles.aboutDropdownBorder]}
//                 onPress={() => { setAboutDropdownVisible(false); router.push(item.route as any); }}
//               >
...
//   aboutDropdownMenu: { position: 'absolute', backgroundColor: '#fff', borderRadius: 8, width: 200, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 5, overflow: 'hidden' },
...
//   signInDropdown: { position: 'absolute', backgroundColor: '#fff', borderRadius: 8, paddingVertical: 12, minWidth: 230, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.15, shadowRadius: 12, elevation: 10, borderWidth: 1, borderColor: '#eee' },
...
//   loginBox: { backgroundColor: '#fff', borderRadius: 14, padding: 32, width: Platform.OS === 'web' ? 450 : '90%', maxWidth: 580, alignItems: 'center', position: 'relative' },
...
// });
// 📁 app/sub-category/SubCategoryScreen.tsx
// ✅ Firebase se tab-wise products fetch karta hai
...
export default function SubCategoryScreen({ categoryId }: { categoryId: any }) {
  ...
  // Cache — baar baar Firebase call na ho ek hi tab ke liye
  ...
  // ============================================================
  // ✅ Firebase se tab ke products fetch karo
  // ============================================================
  useEffect(() => {
    fetchProductsByTab(activeTab);
  }, [activeTab]);

  const fetchProductsByTab = async (tab: string) => {
    // Cache check
    ...
    try {
      // Firestore query: "products" collection mein jahan tab == activeTab
      const q = query(collection(db, 'products'), where('tab', '==', tab));
      ...
      cache.current[tab] = fetched; // cache mein save karo
      setProducts(fetched);
    } catch (error) {
    ...
    <View style={styles.container}>
      ...
      {loading ? (
        <View style={styles.loaderContainer}>
          ...
          <Text style={styles.loadingText}>Loading {activeTab} products...</Text>
        </View>
      ) : (
        <FlatList
          data={products}
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>Is category mein abhi koi product nahi hai</Text>
              <Text style={styles.emptySubText}>"{activeTab}" tab ke liye products upload karein</Text>
            </View>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  // ✅ Loader & Empty State
  loaderContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: 60 },
  loadingText: { marginTop: 12, fontSize: 14, color: '#888' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: 80 },
  emptyText: { fontSize: 15, color: '#999', marginTop: 12, fontWeight: '600' },
